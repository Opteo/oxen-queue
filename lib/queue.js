...
const murmurhash = require('murmurhash') //todo: replace with crypto native lib (maybe md5?)
...
const queue = class {
    constructor({
        ...
        /* Database */
        this.db = mysql.createPool(mysql_connection).promise()
        this.db_table = db_table

        /* Polling */
        this.polling_rate = min_polling_rate
            ...
            [
                jobs.map(job => {
                    ...
                    const unique_key = _.isUndefined(job.unique_key)
                        ? null
                        : murmurhash.v3('' + job.unique_key)

                    const created_ts = job.start_time
    ...
    async doWork(work_fn) {
        ...
        return Promise.resolve(work_fn(job.body))
            .timeout(_this.job_timeout_seconds * 1000, 'timeout on work_fn(job.body)')
